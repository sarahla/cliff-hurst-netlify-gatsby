...
const globalSceneOptions = { triggerHook: "onCenter" }

const FeatureGrid = ({ gridItems }) => (
  gridItems ? (
  <Controller globalSceneOptions={globalSceneOptions}>
    {gridItems.map((item, index) => (
      <Scene key={item.url} classToggle="in-view" reverse={false}>
        <section key={item.url} className={`block ${index % 2 ? 'block--right' : 'block--left'} u-pt-10 u-pb-10 u-mb-20 m-u-mb-5 animate`}>
