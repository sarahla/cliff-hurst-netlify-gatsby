...
const Navbar = (props) => {
  ...
  return (
    <header className={`nav ${active ? navBarActiveClass : ''}`}>
      { !hideLogo ? (
      ...
      <button className="nav__btn--open" data-nav-open="" onClick={() => toggleHamburger()}>
        <svg className="icon-menu"><use xlinkHref={`${icons}#icon-menu`}></use></svg>
      </button>

      <nav className="nav__slideout">
        <button className="nav__btn--close" onClick={() => toggleHamburger()}>
